...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: account, error: accountError } = await supabase
      ...
      .select('page_access_token, instagram_account_id')
    ...
    // Build message payload for Instagram Send API
    ...
    // Send message via Facebook Graph API (Instagram uses the same endpoint)
    const response = await fetch(
      `https://graph.facebook.com/v21.0/${account.instagram_account_id}/messages?access_token=${account.page_access_token}`,
      {
        method: 'POST',
        headers: {
