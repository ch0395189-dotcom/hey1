    ...
    <div className="flex-1 min-h-0 flex flex-col overflow-hidden bg-background h-full">
      ...
      >
        <div className="flex items-center gap-2 md:gap-3 min-w-0 flex-1">
          {onBack && (
            <button
              className="md:hidden shrink-0 p-4 -ml-2 rounded-full hover:bg-primary-foreground/10 active:bg-primary-foreground/20 transition-colors self-center"
              onClick={onBack}
              aria-label="Volver"
            >
      ...
      {(isRecording || audioUrl) && (
        <div className="px-4 py-3 border-t border-border bg-muted/50 safe-area-bottom">
          <div className="max-w-3xl mx-auto flex items-center gap-3">
            {isRecording ? (
              <>
      ...
      <form onSubmit={handleSendMessage} className="shrink-0 p-2 md:p-4 border-t border-border bg-card safe-area-bottom">
        <div className="max-w-3xl mx-auto flex items-end gap-1.5 md:gap-2">
          ...
          />
          {newMessage.trim() && conversation?.platform === 'whatsapp' && (
            <Button
              type="button"
              size="icon"
              variant="outline"
